...
int convert(const char length[]) {
    ...
    for (size_t index = 0; length[index] != 0; index++){
        if (length[index] >= L'0' && length[index] <= L'9'){
            value *= 10;
...
int triangle(int a, int b, int c) {
    ...
    long long longest = 0, small1 = 0, small2 = 0;
    if (a >= b and a >= c){
        longest = a;
        small1 = b;
        small2 = c;
    ...
    if (longest == small1 and longest == small2){
        return Equilateral;
    }
    else if (longest == small1 or longest == small2 or small1 == small2){
        return Isosceles;
    }
    else if (small1 * small1 + small2 * small2 == longest * longest){
        return Right;
    }
    else if (small1 + small2 == longest){
        return Flat;
    }
    else if (small1 + small2 < longest){
        return Impossible;
