...
int convert(int n, const char mark[]) {
    int grade = 0;
    if (n > 1 && mark[0] == '0'){
        // first digit must not be 0, except "0"
        return -1;
    }
    for (unsigned short index = 0; index != n; index++) {
        if (!(mark[index] >= 48 && mark[index] <= '9')){
            // verify all digits are numbers
            return -1;
        }
        grade *= 10; // iterate from highest digit to lowest, so multiply the calculated value by 10
        grade += mark[index] - '0'; // add current digit's value
    }
    return ((grade >= 0 && grade <= 100) ? grade : -1); // three arity operator to verify the grade is in range
}
